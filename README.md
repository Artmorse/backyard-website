...
| --- | --- | --- |
| `create-form-20260926.gs` | registering for an edition that has a date | the edition's `registration.url` |
...
The registration script carries the date of the edition it inscribes to, the
same way `static/gpx/` does: every loop has its own questions, so the next one
gets its own script next to this one rather than overwriting it. The mailing
list outlives the editions and stays undated.

### Their shared look — one manual step

Apps Script cannot dress a form: the `Form` class has no theme, colour or
header-image method, and the Forms REST API has none either. The palette icon
(🎨) in the Google Forms toolbar is the only way in, so a freshly generated form
arrives in Google's default purple.

Only one thing has to be set there: the **header image**, uploaded from
`assets/img/banner.png`. Forms picks the theme and background colours out of the
image itself, so the green of the banner lands on both forms without a single
hexadecimal value being typed — which is also why both forms end up looking
alike from the same upload. Forms crops the image to a band far wider than the
original 1774 × 887, so check the framing after uploading.

Re-running a script creates a **new** form, and that new form comes out
undressed — the image has to be uploaded again. It is the main reason to prefer
editing a generated form directly for small changes.

---

## Creating the registration form (Google Form)

The form is generated by the `create-form-20260926.gs` script through Google
Apps Script. There is no need to build it by hand.

### Steps

1. Go to [script.google.com](https://script.google.com) → **New project**.
2. Copy the whole content of
   [`create-form-20260926.gs`](./create-form-20260926.gs) into the editor.
...
6. Open the edit link and upload the header image, as described in
   [their shared look](#their-shared-look--one-manual-step).
